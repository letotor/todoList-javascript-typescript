...
function toggleTodo(todos: Todo[], index: number) {
...
function toggleTodoChecked(todos: Todo[], index: number) {
  if (index > todos?.length - 1) throw new Error("index out of range");
  todos[index].isChecked = !todos[index].isChecked;
}

export { addTodo, editTodo, deleteTodo, toggleTodo, toggleTodoChecked };
